...
fn add_tabs(window:&Window){
    ...
    let tab_label = gtk::Label::new("tab label").unwrap();
    let menu_label = gtk::Label::new("menu label").unwrap();
    notebook.append_page_menu(&content1, Some(&tab_label), Some(&menu_label));
    window.add(&notebook);
}
